...
type ServiceMode = 'subscription' | 'lifetime';
type CardCycle = 'monthly' | 'yearly';
...
interface SubscriptionRecord {
  ...
  planId: Plan | { _id?: string; name?: string; billingCycle?: string; price?: number };
}
...
function PlanCard({
  ...
  const cardInner = (
    ...
    >
      ...
      <div className={cn((featured || showCycleToggle) && 'mt-4')}>
        ...
        {group.description && (
          <p className="mt-1.5 text-sm leading-snug text-gray-500 line-clamp-2">{group.description}</p>
        )}
      </div>
      ...
      {/* CTA — pinned to bottom when cards stretch */}
      <button
        type="button"
        disabled={isCurrent}
  ...
  return (
    ...
    >
      {featured ? (
        <div className="absolute -inset-px rounded-2xl bg-gradient-to-b from-primary/15 to-transparent pointer-events-none" />
      ) : null}
...
export default function AdminSubscription() {
  ...
  const { data: plans, isLoading: plansLoading } = useQuery({
    ...
    queryFn: async () => (await api.get('/admin/plans')).data.data as Plan[],
    staleTime: 0,
  });
  ...
  if (statusLoading || subLoading || plansLoading || configLoading) {
    return (
  ...
  const currentPlan = subscription?.planId as Plan | undefined;
    ...
    <div className="relative min-h-full overflow-hidden bg-white text-gray-900">
      ...
      <div className="relative z-10 px-5 pb-16 pt-10 sm:px-6 sm:pt-12">
        <div className="mx-auto max-w-4xl">
          ...
          >
            {!needsPlan && subscription && currentPlan && (
              <div className="mb-8 inline-flex flex-wrap items-center justify-center gap-3 rounded-2xl border border-gray-200 bg-white px-5 py-3 shadow-sm">
                ...
                {subscription.currentPeriodEnd && currentPlan.billingCycle !== 'lifetime' && (
                  <>
                    <span className="h-1 w-1 rounded-full bg-gray-300" />
                    <span className="text-xs text-gray-500">Renews {formatDate(subscription.currentPeriodEnd)}</span>
                  </>
                )}
                <span className="h-1 w-1 rounded-full bg-gray-300" />
          ...
          <div className="mt-12 sm:mt-14">
            {visibleGroups.length === 0 ? (
              ...
              <div
                className={cn(
                  'grid gap-6 lg:gap-8',
                  visibleGroups.length === 1 ? 'mx-auto max-w-[380px]' : 'md:grid-cols-2 max-w-4xl mx-auto'
                )}
              >
