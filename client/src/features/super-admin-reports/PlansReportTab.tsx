...
function exportLedgerCsv(rows: PlansReportResponse['ledger'], from: string, to: string) {
  const headers = ['Plan', 'Units Sold', 'Total Revenue', '% of Total Sales', 'Growth %'];
  ...
  link.href = url;
  link.download = `plan-ledger-${from}-to-${to}.csv`;
  link.click();
...
export function PlansReportTab() {
  ...
  const { data, isLoading, isFetching } = useQuery({
    queryKey: ['super-admin-reports-plans', datePreset, range.from, range.to, search, page, sort],
    queryFn: async () => {
      ...
      return (await api.get('/super-admin/reports/plans', { params })).data.data as PlansReportResponse;
    },
  });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <KpiCard
          title="Total Revenue from Plans"
        ...
        />
        <KpiCard
          title="Average Revenue/Plan"
          value={formatCurrency(summary?.avgRevenuePerPlan ?? 0)}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <ProductRevenueDonut
          ...
          distribution={{
            ...
            topProductName: data?.distribution?.topPlanName ?? '—',
            topProductPercent: data?.distribution?.topPlanPercent ?? 0,
          }}
        />
      </div>
      ...
      <Card glass={false} className="overflow-hidden border border-gray-100 bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-5 py-4">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">Plan Ledger</h2>
              ...
              {ledger.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-4 py-10 text-center text-gray-500">
                    No plan sales found for these filters.
                  </td>
                </tr>
              ) : (
                ledger.map((row, index) => {
                  ...
                  return (
                    <tr key={row.planKey} className="border-b border-gray-50 hover:bg-primary-50/20">
