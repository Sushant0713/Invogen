...
export function FavoriteButton({ assetId, isFavorite, className = '' }: FavoriteButtonProps) {
  return (
    <button
      ...
      className={`inline-flex h-6 w-6 items-center justify-center rounded-md text-gray-400 opacity-0 transition-all hover:bg-amber-50 hover:text-amber-500 group-hover/asset:opacity-100 ${isFavorite ? '!opacity-100 text-amber-500' : ''} ${className}`}
    >
