...
import { type TaxSettings, EMPTY_TAX_SETTINGS, getCombinedGstRate, getIgstRate, resolveLineTaxSettings, PRODUCT_GST_RATE_KEY } from './tax-settings';
...
export function calculateInvoice3LineAmounts(
  ...
  let gst = 0;
  let igst = 0;
  if (lineTax.isEnabled && isInvoice3ColumnVisible(columns, INVOICE3_COL_GST)) {
    if (lineTax.taxDisplayMode === 'igst') {
      igst = roundAmount((taxable * getIgstRate(lineTax)) / 100);
    } else {
      gst = roundAmount((taxable * getCombinedGstRate(lineTax)) / 100);
    }
  }
