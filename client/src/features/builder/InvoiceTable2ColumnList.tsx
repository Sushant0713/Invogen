...
import {
  ...
  getTableTotalWidth,
  isInvoice2FixedColumn,
  removeInvoice2Column,
  reorderInvoice2FlexibleColumns,
  setInvoice2ColumnVisible,
  updateColumnWidthPx,
...
function ColumnWidthSlider({
  ...
  onChange: (next: InvoiceTable2Props) => void;
}) {
  return (
    <div>
    ...
    >
      <div className="flex items-center gap-1">
        ...
        <ColumnVisibilityButton
          ...
          onToggle={() =>
            onChange(setInvoice2ColumnVisible(table, col.id, !visible, taxSettings))
          }
        />
      </div>
    ...
    >
      <div className="flex items-center gap-1">
        ...
        <Input
          ...
          onChange={(e) => onChange(updateInvoice2ColumnLabel(table, col.id, e.target.value))}
          className="flex-1"
        />
        <ColumnTypeSelect
          value={getColumnType(col)}
          onChange={(type) =>
            onChange(updateColumnType(table, col.id, type) as InvoiceTable2Props)
          }
        />
        <ColumnVisibilityButton
          visible={visible}
        ...
        <button
          type="button"
          title="Remove column"
          onClick={() => onChange(removeInvoice2Column(table, col.id))}
...
export function InvoiceTable2ColumnList({
  table,
  onChange,
}: {
  table: InvoiceTable2Props;
  ...
  const taxSettings = useTaxSettings();
  const flexibleBounds = getInvoice2FlexibleColumnBounds(table.columns);
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      onChange(reorderInvoice2FlexibleColumns(table, String(active.id), String(over.id)));
    },
    [onChange, table]
  );
