...
export function CustomerRevenueTrendChart({
  ...
  const chartData = useMemo(() => {
    const filled = fillRevenueSeriesGaps(
      data.map((point) => ({ period: point.period, total: point.revenueCollected, count: 0 })),
      from,
      to,
      groupBy,
    );
    ...
    return filled.map((point) => ({
      ...
      revenueCollected: point.total,
    }));
  }, [data, from, to, groupBy]);

  const yScale = useMemo(
    () => getChartYAxisScale(chartData.map((point) => point.revenueCollected)),
    [chartData],
  );
      ...
      <div className="px-2 pb-5">
        <ResponsiveContainer width="100%" height={height}>
          <AreaChart data={chartData} margin={{ top: 12, right: 20, left: 4, bottom: 4 }}>
            ...
            />
            <Tooltip
              formatter={(value: number) => [formatCompactCurrency(value), 'Revenue Collected (₹)']}
