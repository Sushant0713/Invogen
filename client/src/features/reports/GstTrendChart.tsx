...
export function GstTrendChart({
  ...
  const chartData = useMemo(() => {
    const filled = fillRevenueSeriesGaps(
      data.map((point) => ({ period: point.period, total: point.totalGst, count: 0 })),
      from,
      to,
      groupBy,
    );
    ...
    return filled.map((point) => ({
      ...
      totalGst: point.total,
    }));
  }, [data, from, to, groupBy]);

  const yScale = useMemo(
    () => getChartYAxisScale(chartData.map((point) => point.totalGst)),
    [chartData],
  );
          ...
          <AreaChart data={chartData} margin={{ top: 12, right: 20, left: 4, bottom: 4 }}>
            <defs>
              <linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="#FF7700" stopOpacity={0.28} />
            ...
            />
            <Tooltip
              formatter={(value: number) => [formatCompactCurrency(value), 'Total GST Collected (₹)']}
