...
function exportCustomersCsv(rows: SalesReportResponse['customers'], companyName: string) {
  const headers = [
    'Customer',
    'Total Invoices',
    'Total Sales Value',
  ...
  link.href = url;
  link.download = `${companyName.replace(/\s+/g, '-').toLowerCase()}-sales-report.csv`;
  link.click();
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <KpiCard
          ...
          trend={formatTrendLabel(summary?.expectedRevenueChange, 'vs previous period')}
      ...
      <Card glass={false} className="overflow-hidden border border-gray-100 bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-5 py-4">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">Customer Sales Analytics</h2>
            <p className="text-sm text-gray-500">Sales performance by customer for the selected period</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              variant="outline"
              size="sm"
              onClick={() => exportCustomersCsv(customers, companyName)}
